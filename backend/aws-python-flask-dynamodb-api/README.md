...
Additionally, the template also handles provisioning of a DynamoDB database that is used for storing data about users. The Flask application exposes two endpoints, `POST /users` and `GET /user/{username}`, which allow to create and retrieve users.
...
```bash
curl --request POST 'https://xxxxxx.execute-api.us-east-1.amazonaws.com/dev/users' --header 'Content-Type: application/json' --data-raw '{"name": "John", "username": "someusername"}'
```

Which should result in the following response:

```bash
{"username":"someusername","name":"John"}
```

You can later retrieve the user by `username` by calling the following endpoint:

```bash
curl https://xxxxxxx.execute-api.us-east-1.amazonaws.com/dev/users/someusername
```
...
```bash
{"error":"Could not find user with provided \"username\""}
```
